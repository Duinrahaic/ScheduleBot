...
namespace SchedulingAssistant.Services
{
    internal class InteractivityHandler
    {
        ...
        {
            _logger.LogInformation($"{e.Interaction.GuildId} \"{e.Interaction.User.Username}\" made an interaction.");
                ...
                using (var db = new DBEntities())
                {
                    ...
                    switch (CommandMatch.Groups[1].Value)
                    {
                        case "DeleteConfirmation":
                            ...
                            {
                                if (e.Values["text"].ToLower().Trim() == dbSchedule.EventTitle.ToLower().Trim())
                                {
                                    _logger.LogInformation($"User {e.Interaction.User.Username} is has removed an event");
                ...
                using(var db = new DBEntities())
                {
                    ...
                    switch (CommandMatch.Groups[1].Value)
                    {
                        case "Delete":
                            ...
                            if (!DiscordMember.Roles.Any(x => x.CheckPermission(Permissions.ManageMessages | Permissions.Administrator | Permissions.All) == PermissionLevel.Allowed))
                            {
                            ...
                            var Builder = new DiscordInteractionResponseBuilder().WithTitle("Delete Confirmation").WithCustomId($"DeleteConfirmation_{dbSchedule.Id}").WithContent($"{e.User.Mention}, Do you really want to delete event '{dbSchedule.EventTitle}?'");

                            Builder.IsEphemeral = true;

                            Builder.AddComponents(new TextInputComponent($"Type in \'{dbSchedule.EventTitle}\' to delete event:", "text", placeholder: "", required: true, style: TextInputStyle.Paragraph));
